 ...
 * to the system's strict {@link Mapper}, {@link Reducer} or (optional) {@link Combiner} interfaces.
 * </p>
 *
...
public class DynamicClassLoader {
    ...
    /**
     * Dynamically loads and instantiates a Combiner implementation from the filesystem.
     * <p>
     * Architecturally, a Combiner executes a localized reduction phase prior to network
     * shuffling. It is strictly validated against the dedicated {@link Combiner} interface
     * to prevent non-commutative logic from executing mid-stream.
     * </p>
     *
     * @param directoryPath The local path where the bytecode was staged by the Orchestrator.
     * @param className     The fully qualified name of the user's Combiner class.
     * @return An instantiated and validated {@link Combiner} object.
     * @throws IllegalArgumentException If the loaded class does not implement the Combiner interface.
     * @throws Exception                If the class cannot be found or instantiation fails.
     */
    public static Combiner loadCombiner(String directoryPath, String className) throws Exception {
        logger.info("Sandbox: Attempting to load Combiner class '{}' from {}", className, directoryPath);

        Class<?> loadedClass = loadClassFromFile(directoryPath, className);

        // Enforce strict Type Safety against the new Combiner interface
        if (!Combiner.class.isAssignableFrom(loadedClass)) {
            logger.error("Sandbox Validation Error: Combiner class '{}' does not implement the Combiner interface.", className);
            throw new IllegalArgumentException("The provided Combiner class does not implement the Combiner interface.");
        }

        return (Combiner) loadedClass.getDeclaredConstructor().newInstance();
    }
